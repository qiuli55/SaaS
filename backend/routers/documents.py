...
SYSTEM_PROMPT = """你是一名执业律师，请直接输出一份完整的{doc_type}法律文书。

必须遵守的规则：
1. 直接输出文书正文，不要添加任何开场白、解释、说明或结语
2. 不要使用分隔线、星号、破折号等装饰符号
3. 不要写"以下是为您生成的文书"之类的话
4. 不要对文书内容做任何评价或总结
5. 当事人信息必须使用用户提供的精确信息，缺失信息写"不详"
6. 引用法条格式：《中华人民共和国民法典》第XXX条
7. 事实与理由部分基于用户提供的事实撰写，不编造
...
async def generate_document(
    ...
    # 构建提示词
    user_prompt = f"""请根据以下信息撰写一份{req.doc_type}，直接输出文书正文，不要添加任何解释说明。
...
原告：{entry.plaintiff_name}
被告：{entry.defendant_name}
标的额：{entry.amount or '未填写'}元
诉讼请求：{req.claims or '请根据案件事实合理归纳'}
管辖法院：{req.court_name or '有管辖权的人民法院'}
案件事实：{entry.custom_facts or req.facts or '请根据当事人信息合理撰写'}"""
